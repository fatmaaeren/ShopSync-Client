...
function ShoppingListCover({ shoppingList }) {
    ...
    const handleDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = instance.delete(`/ShoppingListDelete?shoppingListId=${shoppingList.shoppingListId}`)
                    setTimeout(() => {
    ...
    return (
        <div className='py-4 px-6 bg-white drop-shadow-xl rounded-lg w-full flex justify-between items-center'>
            <div className='text-sky-900 text-xl'>{shoppingList.name}</div>
            <div className='flex items-center gap-3'>
                <Link
                    to={`/list/${shoppingList.shoppingListId}`}
...
export default ShoppingListCover