...
function LoginPage() {
    ...
    const { values, errors, touched, isSubmitting, handleBlur, handleChange, handleSubmit } = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const response = await axios.post('https://localhost:7199/api/ShopSync/Login', data, {
                    headers: headers,
                ...
                if (response.data.id == 0) {
                    ...
                    const jwtToken = response.data.token.accessToken;
                ...
                <div className='sm:mx-auto sm:w-full sm:max-w-md'>
                    <div className='bg-white py-6 px-4 sm:px-10 rounded-lg' >
                        ...
                        <div className='text-slate-500 text-sm text-center my-4'>
                            New to ShopSync? <Link className='text-rose-500 underline' to={"/register"}>Create an account</Link>
                        </div>
                    </div>
                </div>
            </div>
